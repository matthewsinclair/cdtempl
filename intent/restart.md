---
verblock: "09 Aug 2026:v2.0: Matthew Sinclair - Globalfold: orientation only; the narrative and the ledger move to their own homes"
---
...
**Cdsync v1 is released, public, and every steel thread is Completed.** Ten commands. The bats suite is green end to end and prints its own count; shellcheck silent at default severity; CI green on hygiene, ubuntu-latest and macos-latest, read per job. **Read the spec-library numbers from `cdsync doctor`**, never from prose.
...
**Nothing is open, and nothing awaits hv.** That is the finished state, not a gap. **The next piece of work opens a thread** (`intent st new`) rather than resuming one -- and the carried-forward list below is what to read first.
...
**This is where the remainder went when ST0003 closed.** None of it is queued. **Do not pick one up because it is written down** -- it is recorded so it is not lost, which is the opposite of a queue.

- **WP-02, rule 4's three noise shapes.** Parked by hv, evidence and recommendations on record in its `info.md`. **Nothing moves until a live venture trips one.**
...
- **Unpushed work exists in four repositories and pushing is hv's. Never write the count down** -- there are two remotes at different positions, and **the mirror holding the only copy of the pre-public history is the further behind of the two**. The board's two-remotes watch-out has the commands and the reason.
