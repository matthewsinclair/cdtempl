...
Cdtempl's declared language is **shell**, and this tree is written for it. A module is a file under `lib/` sourced by `bin/cdtempl`.

## Step 1: Is it a new command, or part of one that exists?

**A new user-facing command** -> `lib/cmd_<name>.sh`, plus a dispatch arm in `bin/cdtempl` and a `help/<name>.md`. All three, or the command is undiscoverable.

**Behaviour inside an existing command** -> that command's own `cmd_*.sh`. Do not reach into another command's module: `bin/cdtempl` sources exactly one `cmd_*.sh` per invocation, so a cross-module reference resolves to an unset variable at runtime rather than failing at load.
...
| ----------- | ---------- |
| `cdtempl.json` | `lib/config.sh` |
| Resolving `$CDTEMPL_TARGET` | `lib/target.sh` |
...
**A constant two commands must agree on** -> `lib/common.sh`, always. This is not a style preference: on-demand sourcing means the second command literally cannot see the first one's copy, so the alternative is two definitions that drift. `CDTEMPL_CLASSIFICATION_ONLY` moved there for exactly this reason -- `check` validates the words `bootstrap` instructs on.
...
| ---------- | ---------------- |
| Business logic in `bin/cdtempl` | The command's `cmd_*.sh`. The dispatcher parses and delegates, nothing else |
...
| Writing anything outside `$CDTEMPL_TARGET` | Nowhere. The target is the entire output boundary |
...
Yes. The suite is `test/cdtempl.bats`.
