...
heartbeat_at: 2026-08-06T08:50Z
status: active
...
Completed narrative is archived per day at `.history/`; 2 August is the most recent. This board is live state only.

**Read `intent/docs/design-system-lifecycle.md` first. It is canon and outranks everything here.** Then `intent/restart.md`.

## Where things stand

**Nothing is in flight, and no thread is open.** ST0001 and ST0002 were both closed on 2 August through `intent st done`, which refuses while a contract is BLOCKED -- so each earned it, 16/16 and 12/12. They now live at `intent/st/COMPLETED/`. Do not go looking for half-finished work.

**Cdsync is pristine locally.** 328 tests, 0 failures, shellcheck silent at default severity, docs verified against the as-built rather than assumed.

**CI is green on the final state.** Run `30743668444`, sha `aa987b0`, **success on hygiene, ubuntu-latest and macos-latest** -- the first run covering the rule-4 documentation, both thread closes and ST0003. Checked per job rather than read off the summary line. Only the localfold commit follows it, and it is documentation only.

**hv has put Cdsync to bed until it is needed again.** ST0003 holds every outstanding item, and **picking one up unasked is not the job.**

**Both new spikes completed round one.** matthewsinclair and geodica were installed, checked and committed on 2 August -- the first round either has ever run, and the first real exercise of the cold-start document. **It worked**: both trees came back Cdsync-shaped.

**hv is rolling both design systems out in Laksa now, and fixing forward.** The handoff note is committed at `../Laksa/HANDOFF-design-systems-20260802.md`. Not cc's thread; do not start implementation work here.

## TODO

**All of it now lives in ST0003, "Post-release 0.1.0 clean-up"** -- `intent/st/NOT-STARTED/ST0003/`, eight work packages, each with its own `info.md`. This board no longer carries a second copy: **when a file carries two lists of the same kind of thing, that is the bug, not the style.**

**Unblocked today:** WP-01 cut the release, WP-07 integrate the original four (bar Lamplight), WP-08 housekeeping.

**Waiting on hv:** WP-02 the two rule-4 rulings, WP-05 where `cdsync.json` lives and `spec_version` for a new asset, WP-06 round three's seven rulings plus finding 5.

**Waiting on hv transport:** WP-04, the three port briefs and the conversion behind them.

## Wants hv

**Every ruling this board was holding now lives in ST0003 with its evidence** -- WP-02 (the two rule-4 rulings), WP-05 (where `cdsync.json` lives, `spec_version` for a new asset), WP-06 (round three's seven rulings plus finding 5). Kept there rather than here, for the same reason the TODO list went: two lists of one kind of thing is the bug.

**Off the list rather than settled, so do not re-open:** rule 5 shipped 1 August; rule 3 came off because its residual is mostly TRUE positives -- one of six findings is noise, and suppressing `alt` text to clear it was not worth it.

**Still only on this board, because they belong to no WP:**
...
- **Downloads housekeeping** -- `claude-design-superseded-rounds-202607/` 1.3G, `lamplight-design-drops/` 1.1G, `Lamplight Design System/` 281M, `baize-repo-snapshots-202606-202607/` 233M. **`baize-design-drops/` 441M stays**, the off-repo home named in Baize's own root `.gitignore`. **Six spent `_inbox/` directories now** -- and the two newest hold the Laksa theme packs, **which exist nowhere else**. Do not clear those two until Laksa has taken them.
- **No `hv` node here, by hv's ruling -- `cc` is the whole roster in Cdsync for now**, so rulings arrive in conversation and land in `## Decisions`. The whiteboard `README.md` roster stays deferred.
...
**1. The mutation does not apply, and the test reports the reassuring answer.** Four times on 1 August: `sed 's|...||...|'` against a pattern containing `||`; `perl \Q...$var...\E` interpolating before quoting; `grep -v` on a literal, twice. **Print proof it landed.** `awk -v n="$LN" 'NR!=n'` by line number has not lied yet.
