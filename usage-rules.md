...
This file is the prescriptive DO / NEVER contract for working in Cdsync. Narrative, rationale, and FAQs live in `AGENTS.md` and project technical docs. Each file has one job.

This is Cdsync's own hand-authored contract -- not the `usage_rules` Hex library's per-dependency `deps/<pkg>/usage-rules.md` files, which share the filename but are generated by that tool. See `intent/docs/working-with-llms.md` for the interop.
