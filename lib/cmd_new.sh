...
#
# ALL application knowledge lives in this module. Nothing else in cdtempl knows
...
CDTEMPL_VENTURE_TEMPLATES="cdtempl.json AGENTS.md CLAUDE.md README.md"
...
cmd_new() {
  ...
  if [[ -z "$name" ]]; then
    error "usage: cdtempl new <name> [--target PATH]"
    return 2
  fi
  ...
  local templates="$CDTEMPL_HOME/templates/venture"
  if [[ ! -d "$templates" ]]; then
    error "cannot find the venture templates at $templates"
    echo "  CDTEMPL_HOME resolved to $CDTEMPL_HOME -- is the installation intact?" >&2
    return 1
  fi
  ...
  # The target location first, because cdtempl.json lives at the tree root --
  ...
  # directory, and an absent assets/ makes `cdtempl check` report "no drop here"
  ...
  local file dst
  for file in $CDTEMPL_VENTURE_TEMPLATES; do
    dst="$root/$file"
    if [[ "$file" == "$CDTEMPL_CONFIG_NAME" ]]; then
      dst="$target/$file"
    fi
  ...
  # and design/ only, so a tree anywhere else needs --target or $CDTEMPL_TARGET
  # on every command. Said at the one moment the choice is being made.
  case "$target_rel" in
    design|design/system) ;;
    *) warn "non-standard target: later commands need --target $target_rel or \$CDTEMPL_TARGET -- the probe looks in design/system/ and design/ only" ;;
  esac
  ...
  echo ""
  info "next: fill in ${target#"$PWD"/}/cdtempl.json -- especially 'fixed', 'open' and 'order'"
  info "then: cd $name && cdtempl brief"
  echo ""
...
# ============================================================================
#
# render_venture_template lives in common.sh: `init` renders the cdtempl.json
...
# Generated by cdtempl site. Rebuild it rather than committing it -- it is a view
